...
ChatLogic::~ChatLogic()
{
    ...
    // delete all edges
    //for (auto it = std::begin(_edges); it != std::end(_edges); ++it) //Task 5
    //{
      //  delete *it;
    //}
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode *node) 
                          //                          { return node->GetID() == id; }); //task 3
                      
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> &node) 													
                                                    { return node->GetID() == id; });//task3


                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            //_nodes.emplace_back(new GraphNode(id)); //Task 3
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                          ...
                          auto childNode = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](std::unique_ptr<GraphNode> &node) { return node->GetID() == std::stoi(childToken->second); }); //Task 3
                            ...
                            // create new edge
                            //GraphEdge *edge = new GraphEdge(id);//Task 4
